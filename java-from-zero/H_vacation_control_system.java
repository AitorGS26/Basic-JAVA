...
/*First of all we start importing scanner due to the info the user will provide.
...
public class H_vacation_control_system {
    ...
    program */
        ...
        int key = 0;
        int seniority = 0;
        /*After declaring the variables, we ask for data to the user, saving
         the info in that previously created variables.*/
        System.out.print("What is your name? ");
        name = input.nextLine();//String = nextline()

        System.out.println(""); //space

        System.out.print("What's your key? ");
        key = input.nextInt();//int = nextint()
        ...
        /**
         * Now we've all the things on the table: We've the library we'll be
         * using we've all the variables declared we've tha user's info inside
         * those variables
         *
         * Now the thing is to do the logic of the program, in this case we'll
         * be using the if-else structures we're seeing in provious lessons.
         */
        if (key == 1) {
            if (seniority == 1) {
